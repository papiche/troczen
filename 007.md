...
Après cela, toute tentative de transfert est invalide.
...
```
     ...
     |   stocke P3 caché (neurones du réseau)                   |
     |   réplique les kind 30303, kind 1 et kind 5                |
     -------------------------------------------------------------
                           ^
                           | Sync du matin / borne Wifi
                           |   (P3 caché disponible offline)
                           |
                +----------+-----------+
                |     PORTEUR / CLIENT  |
                |    sk_U / pk_U        |
...
flowchart TD
    ...
    %% Acceptant / commerçant
    subgraph A["ACCEPTANT / COMMERÇANT"]
    ...
    subgraph Final["FINALISATION"]
        ...
        Owned_A["A possède B (Wallet)"]
        Nostr_transfer["Publish kind 1: TRANSFER signé B"]
    end
    ...
    subgraph LEGEND["Légende"]
        ...
        P1_legend["P1 = Ancre (Émetteur)"]:::tamarin
        P2_legend["P2 = Voyageur (Porteur)"]:::tamarin
        P3_legend["P3 = Témoin (Réseau)"]:::tamarin
        QR_legend["QR1/QR2 = double scan atomique"]
        Kind_legend["Kind 30303 = Création, Kind 1 = TRANSFER"]
    end
```
